...
27.05.2024 - V3.0
Atualizações: 
Implementando sensor de turbidez
Inserindo parte do código de leitura no código fonte
O sensor faz a leitura e insere no TagIo normalmente
Criado o Json3 para leitura de turbidez   
*/
// Bibliotecas
...
void delayedFunction() {
  ...
  if (analogValue > 450) { // ESSE VALOR DEPENDE PARA CADA GRUPO - CALIBRAR
    Serial.println("LIMPA"); //ESSA STRING VAI PARA O TAGO.IO
  }
  if ((analogValue > 250) && (analogValue < 450)) { //ESSES VALORES DEPENDEM PARA CADA GRUPO - CALIBRAR
    Serial.println("UM POUCO SUJA"); //ESSA STRING VAI PARA O TAGO.IO
  }
  if (analogValue <= 250) { //ESSES VALORES DEPENDEM PARA CADA GRUPO - CALIBRAR
    Serial.println("MUITO SUJA"); //ESSA STRING VAI PARA O TAGO.IO
  }
